...
\newcommand{\popA}{\textsc{Pop\textsubscript{A}}}
\newcommand{\popB}{\textsc{Pop\textsubscript{B}}}
...
\newcommand{\comp}{\textsc{Peak+Continuum:Spin} model}
...
\begin{abstract}
    Observations of gravitational waves (GWs) from merging compact binaries have become a regular occurrence. The continued advancement of the LIGO-Virgo-KAGRA (LVK) detectors have now produced a catalog of over 90 such mergers, from which we can begin to uncover the formation history of merging compact binaries. In this work, we search for mass-spin correlations in the LVK's third gravitational wave transient catalog through the use of data-driven mixture models in a hierarchal Bayesian inference framework. We find an over density of mergers with a primary mass of $\result{\CIPlusMinus{\macros[Mass][Base][PeakA][max]}\msun}$, with properties consistent with those predicted by isolated binary formation and stable mass transfer. These low-mass events have a spin magnitude distribution peaking at $a_\mathrm{peak}=$ \result{$\CIPlusMinus{\macros[SpinMag][Base][PeakA][max]}$}, exhibits spins preferentially aligned with the binary's orbital angular momentum, is constrained by \result{$\CIPlusMinus{\macros[NumEvents][Base][PeakA]}$} of our observations, and contributes \result{$\CIPlusMinusPer{\macros[BranchingRatios][Base][PeakA][Percent]}$} to the overall population of BBHs. We find a $\sim\macros[FracCut][BgreaterA]\%$ chance that the bulk of the events in the $15-55\msun$ range share a spin distribution with the $10\msun$ peak, with $99\%$ of these events possessing primary masses less than $m_{1,99\%} = $ \result{$\CIPlusMinus{\macros[Mass][Composite][ContinuumA][99percentile]}\msun$}, which provides an estimate of the lower edge of the theorized pair instability mass gap. Additionally, our results hint towards BBHs above $60\msun$ having source properties distinct from the lower mass systems. This work is a first step in gaining a deeper understanding of compact binary formation and evolution with data-driven models, and will provide more robust conclusions as the catalog of observations becomes larger.
\end{abstract}
