...
\begin{itemize}
    ...
    \item \contB{}, $M=2$ (\jaxen{NUMBER} parameters). The spin models have the same form as \first{}, but here primary mass is fit to a B-spline function. 
...
\begin{itemize}
    \item \first{}, $M=1$ (\jaxen{NUMBER} parameters). This category assumes a truncated Log-Normal model in primary mass, a B-spline model in spin magnitude $a_i$, and a B-spline model in $cos(\theta_{\text{tilt}_i})$. 
    \begin{eqnarray} \label{eq:lowmass}
    ...
    \item \contA{}, $M=2$ (\jaxen{NUMBER} parameters). The spin models are the same as the previous category, but here primary mass is fit to a B-spline function. 
    \begin{eqnarray} \label{eq:contmass}
        \text{log} p_{m}(m_1| \Lambda_{m}) = B_k(m_1 | \mathbf{c}_{m, 2}) \\
        p_{a}(a_j| \Lambda_{a}) = B_k(a_j | \mathbf{c}_{a,2}) \\
        p_{\theta}(cos(\theta_j)| \Lambda_{\theta}) = B_k( cos(\theta_j) | \mathbf{c}_{\theta,2})
    \end{eqnarray}
    
    \item \contB{}, $M=3$ (\jaxen{NUMBER} parameters). Here, both primary mass and spin are fit to B-Splines.
    \begin{eqnarray} \label{eq:contmass}
