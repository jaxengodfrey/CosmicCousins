...
Using the \base{}, we infer the location of \popA{}'s primary mass peak to be \result{$\CIPlusMinus{\macros[Mass][Base][PeakA][max]} \msun$} while \popB{} accounts for the rest of the mass spectrum. Figure \ref{fig:g1_mass_distribution} shows the primary mass distributions of the \base{}, as well as the total primary mass distribution inferred by \brucepaper{} which is statistically consistent with the \base{}. 

Figure \ref{fig:mass_ratio_distribution} shows the mass ratio distributions of \popA{} and \popB{}. The distributions are broadly consistent with each other, though \popB{} appears to be more consistent with a power law (the LVK's fiducial mass ratio model) than \popA{}. This is apparent when considering the estimated maximum density location of \popA{}, which excludes $q=1$ while \popB{} does not completely rule it out. 

Looking to Figure \ref{fig:spin_distributions}, we find that \popA{} is more consistent with lower spin magnitudes than \popB{} and supresses maximally spinning systems. Most significantly, \popA{} heavily favors aligned systems and is inconsistent with an isotropic spin tilt distribution. It peaks at $\cos{\theta}=$ \result{$\CIPlusMinus{\macros[CosTilt][Base][PeakA][max]}$} and has a fraction of negative tilts $f_{\cos{\theta} < 0} = $ \result{$\CIPlusMinus{\macros[CosTilt][Base][PeakA][negfrac]}$}. In contrast, \popB{} does not appear to favor aligned systems and does not exclude isotropy. Its tilt distribution peaks at $\cos{\theta}=$ \result{$\CIPlusMinus{\macros[CosTilt][Base][ContinuumB][max]}$} and has a fraction of negative tilts $f_{\cos{\theta} < 0} = $ \result{$\CIPlusMinus{\macros[CosTilt][Base][ContinuumB][negfrac]}$}.

Not only are the source properties of \popA{} distinct from \popB{}, all events can be confidently placed in either \popA{} or \popB{}. Of the 69 total events in the catalog, 54 have a probability $> 0.99$ of belonging to \popB{}. Of the remaining 15 events, all are likely to belong to \popA{} with 14 have a probability well above $0.9$ of belonging to \popA{} and one has a probability of $0.88$. This clear distinction between events in \popA{} and \popB{} underscores their differences in source properties. 
