...
for i in range(len(cm)):
    ...
    x = idata.posterior['mass_1_obs_event_{}'.format(num)].values[0]

    # creating new axes object
    ax_objs.append(fig.add_subplot(gs[i:i+1, 0]))

    # plotting the distribution
    sns.kdeplot(x=x, ax=ax_objs[-1], color=hex[num], lw=1, fill=True, multiple = 'stack')
    ...
    if i == len(hex)-1: 
        ax_objs[-1].set_xlabel(r"$m_1$", fontsize=12)
        logticks = np.array([5,8,10,20,40,70,100])
        ax_objs[-1].set_xticks(logticks)
        ax_objs[-1].get_xaxis().set_major_formatter(ScalarFormatter())
        ...
        spines = ["top","right","left"]
        ax_objs[-1].spines['bottom'].set(alpha = 0.4)
        for s in spines:
            ax_objs[-1].spines[s].set_visible(False)
...
plt.suptitle("Primary Mass Re-Weighed Posteriors")
