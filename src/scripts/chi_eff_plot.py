...
# ax[i_1] = plot_mean_and_90CI(ax[i_1], subpop_ppds['Composite']['PeakA']['chieffs'], subpop_ppds['Composite']['PeakA']['pchieff'], color ='tab:cyan', label='Peak A', bounds = True, lw = 2, line_style = '--', fill_alpha = fill)
...
ax[i_1] = plot_mean_and_90CI(ax[i_1], subpop_ppds['Composite']['ContinuumA']['chieffs'], subpop_ppds['Composite']['ContinuumA']['pchieff'], color ='tab:purple', label='Peak A & Continuum A', bounds = True, lw = 2, line_style = '--', fill_alpha = fill)
ax[i_1] = plot_mean_and_90CI(ax[i_1], subpop_ppds['Composite']['ContinuumB']['chieffs'], subpop_ppds['Composite']['ContinuumB']['pchieff'], color ='tab:pink', label='Continuum B', bounds = True, lw = 2, line_style = (0, (1, 1)), fill_alpha = fill)
ax[i_1].legend(frameon=False, loc = 2, fontsize=legend_text_size);
ax[i_1].set_xlabel(r'$\chi_{eff}$', fontsize=label_text_size)
ax[i_1].set_ylabel(r'$p(\chi_{eff})$', fontsize=label_text_size)
ax[i_1].grid(True, which="major", ls=":")
ax[i_1].set_xlim(-0.75, 0.75)
ax[i_1].set_ylim(0, 5)
# ax[i_1].set_yscale('log')
ax[i_1].set_title(r'Composite Model $\chi_{eff}$ Distributions')
